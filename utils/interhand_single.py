...
def cut_inter_img(loader, save_path, split):
    ...
    idx = 0
    for i in tqdm(range(len(loader))): ### train ### #### 1361062 ### #### 849160 ####
        ...
        if hand_type == 'right' and hand_type_valid:
            mano_dict, coord_dict = loader.load_mano(i)
            if coord_dict['right'] is not None:
                left = coord_dict['right']['verts'][0].detach().numpy()
...
class InterHand_dataset():
    ...
    def __getitem__(self, idx):
        ...
        #     handJ2d = handJ2d[:, :2] / handJ2d[:, 2:]
        #
        #     params['R'] = np.einsum('kj,bji->bki', R, params['R'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default='/mnt/user/E-shenfei.llj-356552/data/dataset/interhand_5fps/')
    parser.add_argument("--save_path", type=str, default='/mnt/user/E-shenfei.llj-356552/data/dataset/interhand_5fps/singlehand/')
