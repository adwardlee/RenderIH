...
class InterHand_dataset():
    ...
    def __getitem__(self, idx):
        cur_idx = idx
        img = cv.imread(osp.join(self.data_path, self.split, 'img', '{}.jpg'.format(cur_idx)))
        hand_dict = np.load(os.path.join(self.data_path, self.split, 'ori_handdict', '{}.npy'.format(cur_idx)),
                            allow_pickle=True)
        hand_dict = hand_dict[()]
...
if __name__ == '__main__':
    ...
    iou_list = './iou_0_27w.npy'
    theiou = np.load(iou_list, allow_pickle=True)
    iou033 = theiou < 0.33
    iou067 = np.logical_and(theiou < 0.67, theiou >= 0.33)
    iou1 = (theiou >= 0.67)
    opt.map = False
    ...
    with torch.no_grad():
        for data in tqdm(dataloader):
            ...
            joints_left_gt = data[1].cuda()
            verts_left_gt = data[2].cuda()
            joints_right_gt = data[3].cuda()
            verts_right_gt = data[4].cuda()
    ...
    print('iou 0.33 mrrpe: {}'.format(mrrpe[iou033].mean()), flush=True)
    print('iou 0.67 mrrpe: {}'.format(mrrpe[iou067].mean()), flush=True)
    print('iou 1 mrrpe: {}'.format(mrrpe[iou1].mean()), flush=True)
    ...
    print('cdev 0.33: {}'.format(nanmean(error[iou033])), flush=True)
    print('cdev 0.67: {}'.format(nanmean(error[iou067])), flush=True)
    print('cdev 1: {}'.format(nanmean(error[iou1])), flush=True)
    ...
    print('orimpjpe 0.33: {}'.format( (orijoint_loss['left'][iou033].mean() * 1000+orijoint_loss['right'][iou033].mean() * 1000)/2 ), flush=True)
    print('orimpjpe iou033 std: {}'.format(
        (orijoint_loss['left'][iou033].std() * 1000 + orijoint_loss['right'][iou033].std() * 1000) / 2), flush=True)
    print('orimpjpe 0.67: {}'.format( (orijoint_loss['left'][iou067].mean() * 1000+orijoint_loss['right'][iou067].mean() * 1000)/2), flush=True)
    print('orimpjpe iou067 std: {}'.format(
        (orijoint_loss['left'][iou067].std() * 1000 + orijoint_loss['right'][iou067].std() * 1000) / 2), flush=True)
    print('orimpjpe 1: {}'.format((orijoint_loss['left'][iou1].mean() * 1000+orijoint_loss['right'][iou1].mean() * 1000)/2), flush=True)
    print('orimpjpe 1 std: {}'.format(
        (orijoint_loss['left'][iou1].std() * 1000 + orijoint_loss['right'][iou1].std() * 1000) / 2), flush=True)
    ...
    print('mpjpe 0.33: {}'.format(
        (joints_loss['left'][iou033].mean() * 1000 + joints_loss['right'][iou033].mean() * 1000) / 2), flush=True)
    print('mpjpe 0.33 std: {}'.format(
        (joints_loss['left'][iou033].std() * 1000 + joints_loss['right'][iou033].std() * 1000) / 2), flush=True)
    print('mpjpe 0.67: {}'.format(
        (joints_loss['left'][iou067].mean() * 1000 + joints_loss['right'][iou067].mean() * 1000) / 2), flush=True)
    print('mpjpe 0.67 std: {}'.format(
        (joints_loss['left'][iou067].std() * 1000 + joints_loss['right'][iou067].std() * 1000) / 2), flush=True)
    print('mpjpe 1: {}'.format(
        (joints_loss['left'][iou1].mean() * 1000 + joints_loss['right'][iou1].mean() * 1000) / 2), flush=True)
    print('mpjpe 1 std: {}'.format(
        (joints_loss['left'][iou1].std() * 1000 + joints_loss['right'][iou1].std() * 1000) / 2), flush=True)
    ...
    print('pampjpe 0.33: {}'.format(
        (pajoints_loss['left'][iou033].mean() * 1000 + pajoints_loss['right'][iou033].mean() * 1000) / 2), flush=True)
    print('pampjpe 0.67: {}'.format(
