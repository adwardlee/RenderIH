...
from easymocap.smplmodel.lbs import batch_rodrigues
...
class ManoLayer(Module):
    def __init__(self, manoPath, center_idx=9, use_pca=True, new_skel=False):
        ...
        print('load {} !!!!! '.format(manoPath), flush=True)
    ...
    def forward(self, root_rotation, pose=None, shape=None, trans=None, scale=None, rotation=None):
        # input
        ...
        if pose is None:
            out_rotation = {'poses':0, 'shapes':0, 'Rh':0, 'Th':0}
            if not torch.is_tensor(root_rotation['poses']):
                out_rotation['poses'] = torch.from_numpy(root_rotation['poses'])
                out_rotation['shapes'] = torch.from_numpy(root_rotation['shapes'])
                out_rotation['Rh'] = torch.from_numpy(root_rotation['Rh'])
                out_rotation['Th'] = torch.from_numpy(root_rotation['Th'])
            else:
                out_rotation['poses'] = root_rotation['poses']
                out_rotation['shapes'] = root_rotation['shapes']
                out_rotation['Rh'] = root_rotation['Rh']
                out_rotation['Th'] = root_rotation['Th']
            pose = out_rotation['poses'][:, 3:]
            shape = out_rotation['shapes']
            if shape.shape[0] < pose.shape[0]:
                shape = shape.expand(pose.shape[0], -1)
            rotation = out_rotation['Rh']
            trans = out_rotation['Th']
            root_rotation = batch_rodrigues(out_rotation['poses'][:, :3])
        ...
        if rotation is not None:
            if len(rotation.shape) == 2:  # angle-axis
                rot = batch_rodrigues(rotation)
            else:
                rot = rotation
            v_output = torch.matmul(v_output, rot.transpose(1, 2))
            j_output = torch.matmul(j_output, rot.transpose(1, 2))
        ...
        ### convert to mm ###
        # v_output = v_output * 1000
        # j_output = j_output * 1000
