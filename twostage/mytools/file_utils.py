...
def read_annot(annotname, height, width, mode='body25'):
    ...
    for i in range(len(data)):
        ...
        for key in ['bbox', 'keypoints', 'handl2d', 'handr2d', 'twohand', 'face2d']:
        ...
        if mode == 'body25':
            ...
            if key not in data[i].keys():
                data[i]['bbox'], data[i]['rectbbox'] = np.array(get_bbox_from_pose(data[i]['keypoints'], height, width, rate=0.6)).astype(np.float32)
            else:
                data[i]['bbox'] = data[i]['bbox_'+mode+'2d'][:5]
        elif mode == 'twohand':
            data[i]['keypoints'] = np.array(data[i]['twohand'].astype(np.float32))
            data[i]['left_kpts'] = data[i]['keypoints'][:21]
            data[i]['right_kpts'] = data[i]['keypoints'][21:]
            data[i]['bbox'], data[i]['rectbbox'] = np.array(get_bbox_from_pose(data[i]['keypoints'], height, width, rate=0.15)).astype(np.float32)
            data[i]['left_box'], _ = np.array(get_bbox_from_pose(data[i]['keypoints'][:21], height, width, rate=0.15)).astype(np.float32)
            data[i]['right_box'], _ = np.array(get_bbox_from_pose(data[i]['keypoints'][21:], height, width, rate=0.15)).astype(np.float32)
        elif mode == 'total':
...
def get_bbox_from_pose(pose_2d, height=None, width=None, img=None, rate = 0.1):
    ...
    dy = (y_max - y_min)*rate
    # 后面加上类别这些
    rect_bbox = [x_min-dx, y_min-dy, x_max+dx, y_max+dy, 1]
    if height is not None and width is not None:
        bbox = [max(0, rect_bbox[0]), max(0, rect_bbox[1]), min(width - 1, rect_bbox[2]), min(height - 1, rect_bbox[3]), 1]
    if img is not None:
        correct_bbox(img, rect_bbox)
    return bbox, bbox#rect_bbox
