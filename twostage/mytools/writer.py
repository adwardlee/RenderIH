...
from .file_utils import write_keypoints3d, write_smpl, mkout, mkdir, write_vertices
...
class FileWriter:
    ...
    def vis_double_smpl(self, render_data, images, cameras, outname, add_back):
        mkout(outname)
        from ..visualize.renderer import Renderer
        render = Renderer(height=1024, width=1024, faces=None)
        render_results = render.double_render(render_data, cameras, images, outname, add_back=add_back)
        image_vis = merge(render_results, resize=not self.save_origin)
        cv2.imwrite(outname, image_vis)
        return image_vis
