...
from ..pyfitting import optimizeMirrorSoft, optimizeMirrorDirect, optimizeSingleSoft
...
def load_weight_mirror(model, opts):
    ...
    elif model == 'manol' or model == 'manor':
        weight = {
            'k2d': 2e-3,
            'init_poses': 1e-3, 'init_shapes': 1e-2,
            'smooth_body': 10, 'smooth_poses': 2, #'smooth_body': 5e-1, 'smooth_poses': 1e-1,
            # 'par_self': 5e-2, 'ver_self': 2e-2,
            # 'par_mirror': 5e-2
        }
    else:
...
def multi_stage_optimize_single(body_model, body_params, bboxes, keypoints2d, Pall, normal, args):
    weight = load_weight_mirror(args.model, args.opts)
    config = Config()
    if args.double:
        config.device = body_model.J_regressor.device
    else:
        config.device = body_model.device
    config.verbose = args.verbose
    config.OPT_R = True
    ...
    with Timer('Optimize 2D Pose/{} frames'.format(keypoints2d.shape[1]), not args.verbose):
        if args.direct:
            ...
            config.OPT_POSE = False
            body_params = optimizeSingleSoft(body_model, body_params, bboxes, keypoints2d, Pall, normal, weight, config, args.double)
            config.OPT_POSE = True
            body_params = optimizeSingleSoft(body_model, body_params, bboxes, keypoints2d, Pall, normal, weight, config, args.double)
    return body_params