...
class ImageFolder:
    ...
    def __getitem__(self, index: int):
        ...
        annot = read_annot(annname, height, width, self.kpts_type)
        return img, annot
    ...
    def camera(self, index=0, annname=None):
        ...
        camera = {'K':K.astype(np.float32) ,'R': np.eye(3).astype(np.float32), 'T': np.zeros((3, 1)).astype(np.float32), 'dist': np.zeros((1, 5)).astype(np.float32)}
        if self.gtK:
            camera['K'] = self.gtCameras[index]['K'].astype(np.float32)
        if self.gtRT:
            camera['R'] = self.gtCameras[index]['R'].astype(np.float32)
            camera['T'] = self.gtCameras[index]['T'].astype(np.float32)
        # camera['T'][2, 0] = 5. # guess to 5 meters
        camera['RT'] = np.hstack((camera['R'], camera['T'])).astype(np.float32)
        camera['P'] = (camera['K'] @ np.hstack((camera['R'], camera['T']))).astype(np.float32)
        return camera
    ...
    def write_smpl(self, results, nf, hand_type='right'):
        outname = join(self.out, 'smpl', '{}.json'.format(self.basename(nf) + hand_type))
    ...
    def vis_double_smpl(self, render_data, image, camera, nf):
        outname = join(self.out, 'smpl', '{}.jpg'.format(self.basename(nf)))
        images = [image]
        for key in camera.keys():
            camera[key] = camera[key][None, :, :]
        self.writer.vis_double_smpl(render_data, images, camera, outname, add_back=True)
