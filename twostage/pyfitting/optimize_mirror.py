...
def optimizeSingleSoft(body_model, params, bboxes, keypoints2d, Pall, normal, weight, cfg, double=0):
    """
        simple function for optimizing mirror
    ...
    if nFrames > 1:
        loss_funcs['smooth_body'] = LossSmoothBody(cfg)
        loss_funcs['smooth_poses'] = LossSmoothPosesMulti([0, nFrames], cfg)
    postprocess_funcs = [
        dict_of_tensor_to_numpy,
        # flipSMPLPosesV
    ]
    params = _optimizeSMPL(body_model, params, prepare_funcs, postprocess_funcs, loss_funcs, weight_loss=weight,
                           cfg=cfg, double=double)
