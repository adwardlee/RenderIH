...
def _optimizeSMPL(body_model, body_params, prepare_funcs, postprocess_funcs, 
    ...
    grad_require(opt_params, True)
    if cfg.OPT_POSE == True:
        optimizer = LBFGS(opt_params, lr=1.5, line_search_fn='strong_wolfe')
    else:
    ...
    def closure(debug=False):
        ...
        if double:
            _, kpts_est = body_model(new_params)
            #kpts_est = kpts_est.numpy()
        else:
    ...
    print('with pose optim {} final loss : {}'.format(cfg.OPT_POSE, loss_dict), flush=True)
