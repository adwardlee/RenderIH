...
from common.utils.loss_utils import DiceLoss
...
class decoder(nn.Module):
    ...
    def forward(self, x, fmaps, targets, mode):
        ...
        batch_size = len(pred_mano_left['joints3d'])
        ...
        if mode == 'train':
            ...
            if self.cfg.normal:
                loss['normal_loss'] = 50 * (
                            normal_loss((pred_mano_left['verts3d'] - pred_mano_left['joints3d'][:, 0:1])[..., :3], (targets['left']['verts3d'] - targets['left']['joints3d'][:, 0:1])[..., :3], self.left_face) + \
                            normal_loss((pred_mano_right['verts3d'] - pred_mano_right['joints3d'][:, 0:1])[..., :3], (targets['right']['verts3d'] - targets['right']['joints3d'][:, 0:1])[..., :3], self.right_face))
            if self.cfg.edge:
                loss['edge_loss'] = 20 * (
                            edge_length_loss((pred_mano_left['verts3d'] - pred_mano_left['joints3d'][:, 0:1])[..., :3], (targets['left']['verts3d'] - targets['left']['joints3d'][:, 0:1])[..., :3], self.left_face) + \
                            edge_length_loss((pred_mano_right['verts3d'] - pred_mano_right['joints3d'][:, 0:1])[..., :3], (targets['right']['verts3d'] - targets['right']['joints3d'][:, 0:1])[..., :3], self.right_face))

            loss['mano_verts'] = cfg.lambda_mano_verts * (F.l1_loss(pred_mano_left['verts3d'] - pred_mano_left['joints3d'][:, 0:1], targets['left']['verts3d'] - targets['left']['joints3d'][:, 0:1]) +
                                 F.l1_loss(pred_mano_right['verts3d'] - pred_mano_right['joints3d'][:, 0:1], targets['right']['verts3d'] - targets['right']['joints3d'][:, 0:1])
                                                          + F.l1_loss(result['v3d_left'] - result['j3d_left'][:, 0:1], targets['left']['verts3d'] - targets['left']['joints3d'][:, 0:1]) +
                                 F.l1_loss(result['v3d_right'] - result['j3d_right'][:, 0:1], targets['right']['verts3d'] - targets['right']['joints3d'][:, 0:1]))
            loss['mano_joints'] = cfg.lambda_mano_joints * (F.l1_loss(pred_mano_left['joints3d']- pred_mano_left['joints3d'][:, 0:1], targets['left']['joints3d']- targets['left']['joints3d'][:, 0:1]) +
                                                            F.l1_loss(pred_mano_right['joints3d'] - pred_mano_right['joints3d'][:, 0:1],
                                                                       targets['right']['joints3d'] - targets['right']['joints3d'][:, 0:1]) +
                                                            F.l1_loss(result['j3d_left'] - result['j3d_left'][:, 0:1], targets['left']['joints3d'] - targets['left']['joints3d'][:, 0:1]) +
                                 F.l1_loss(result['j3d_right'] - result['j3d_right'][:, 0:1], targets['right']['joints3d'] - targets['right']['joints3d'][:, 0:1]))
            loss['joints_img'] = cfg.lambda_joints_img * (
                        ...
                        # )
            if self.cfg.render:
                left_mask = targets['left']['mask']
                right_mask = targets['right']['mask']
                double_mask = 1 - torch.logical_or(left_mask,right_mask)

                v_color = torch.zeros((778 * 2, 3))
                v_color[:778, 0] = 0  # int(1 * 255)
                v_color[:778, 1] = 255  # int(1 * 255)
                v_color[:778, 2] = 0  # int(1 * 255)
                v_color[778:, 0] = 0  # int(0.6 * 255)
                v_color[778:, 1] = 0  # int(0.6 * 255)
                v_color[778:, 2] = 255  # int(0.6 * 255)
                img_out, mask_out = self.renderer.render_rgb_orth(scale_left=scale['left'] * 0.5,
                                                             trans2d_left=trans2d['left'] * 0.5 + 0.5,
                                                             scale_right=scale['right'] * 0.5,
                                                             trans2d_right=trans2d['right'] * 0.5 + 0.5,
                                                             v3d_left=pred_mano_left['verts3d'],
                                                             v3d_right=pred_mano_right['verts3d'],
                                                             v_color=v_color, amblights=True)
                img_out[img_out[:,:,:,2] >= 0.95] = 1
                img_out[img_out[:,:,:,1] >= 0.95] = 1
                img_out = img_out.transpose((0, 3, 1, 2))
                target_mask = torch.zeros(img_out.shape).to(img_out.device)
                target_mask[double_mask,0] = 1
                target_mask[...,1] = left_mask
                target_mask[...,2] = right_mask

                loss['segloss'] = self.diceloss(img_out, target_mask)

            if self.cfg.vert2d:
                loss['verts_img'] = cfg.lambda_joints_img * (
                        F.mse_loss(result['vert2d_left'],
                                   targets['left']['verts2d'].float()) +
                        F.mse_loss(result['vert2d_right'],
                                   targets['right']['verts2d'].float()))
            if not self.mano:
                return loss
            loss['mano_pose'] = cfg.lambda_mano_pose * (F.mse_loss(pred_mano_left['mano_pose'], targets['left']['mano_pose']) +
                                                        F.mse_loss(pred_mano_right['mano_pose'], targets['right']['mano_pose']))
            loss['mano_shape'] = cfg.lambda_mano_shape * (F.mse_loss(pred_mano_left['mano_shape'], targets['left']['mano_shape']) +
                                                          F.mse_loss(pred_mano_right['mano_shape'], targets['right']['mano_shape']))
            ...
            if self.mano:
                out['mano_pose_left'] = pred_mano_left['mano_pose']
                out['mano_pose_right'] = pred_mano_right['mano_pose']
                out['mano_shape_left'] = pred_mano_left['mano_shape']
                out['mano_shape_right'] = pred_mano_right['mano_shape']
            # else:
