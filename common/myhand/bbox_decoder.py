...
class decoder(nn.Module):
    def __init__(self,
        ...
        self.mano = mano_flag
        if self.mano:
            self.param_regressor = ParamRegressor(joint_num=779)
    ...
    def forward(self, x, fmaps, targets, bbox_info, mode):
        ...
        for hand_type in ['left', 'right']:
            ...
            result['verts2d' + '_' + hand_type] = projection_batch(scale[hand_type], trans2d[hand_type], result['v3d' + '_' + hand_type], img_size=cfg.input_img_shape[0])
        ...
        if not self.mano:
            ...
            tmp_left = torch.cat([result['v3d_left'], bbox_info.reshape(-1, 1, 3)], 1)
            tmp_right = torch.cat([result['v3d_right'], bbox_info.reshape(-1, 1, 3)], 1)
            pose_param_left, shape_param_left, pose_rotmat_param_left = self.param_regressor(tmp_left)
            pose_param_right, shape_param_right, pose_rotmat_param_right = self.param_regressor(tmp_right)
        ...
        if mode == 'train':
            ...
            loss['joints_img'] = cfg.lambda_joints_img * (
                        F.mse_loss(pred_mano_left['joints_img'], targets['left']['joints_img'].float()) +
                        F.mse_loss(pred_mano_right['joints_img'], targets['right']['joints_img'].float()) +
                        F.mse_loss(result['j2d_left'],
                                   targets['left']['joints_img'].float()) +
                        F.mse_loss(result['j2d_right'],
                                   targets['right']['joints_img'].float())
                        )
            if not self.mano:
                return loss
...
def load_decoder_cliff(cfg, encoder_info):
