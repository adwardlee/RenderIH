...
class Ego_dataset():
    def __init__(self, data_path, split):
        ...
        self.data_path = '/mnt/user/E-shenfei.llj-356552/workgroup/lijun/hand_dataset/ego3dhands/'

    def __len__(self):
        return self.size

    def __getitem__(self, idx):
        img = cv.imread(osp.join(self.data_path, 'img', '{}.jpg'.format(idx)))
        ...
        data = np.load(os.path.join(self.data_path, 'annot', '{}.npy'.format(idx)), allow_pickle=True)
        data = data[()]

        hand_dict = {}
        hand_dict['left'] = data['left']
        hand_dict['right'] = data['right']
        for hand_type in ['left', 'right']:
            # hms = []
            # for hIdx in range(7):
