...
from common.myhand.lijun_single_model import load_single_model
...
from common.myhand.lijun_vitpose import load_vit
...
from utils.lr_sc import StepLR_withWarmUp, MultiStepLR
from utils.DataProvider import DataProvider
# from utils.vis_utils import mano_two_hands_renderer
from utils.manoutils import get_mano_path, projection_batch

from core.loader import handDataset
from core.loader_mano import handDataset2
from apps.eval_interhand import handDataset as handDataset1
from dataset.interhand import fix_shape, InterHand_dataset
from core.Loss import GraphLoss, calc_loss_GCN
from core.Loss_mano import ManoLoss, mano_loss_GCN
...
def train_gcn(rank=0, world_size=1, cfg=None, dist_training=False, only_j3d=0):
    if dist_training:
    ...
    if 'vit' in cfg.MODEL_NAME:
        network = load_vit(cfg)
    elif 'newgraph' in cfg.MODEL_NAME:
        network = load_new_model(cfg)
    elif 'single' in cfg.MODEL_NAME:
        network = load_single_model(cfg)
    else:
        network = load_graph_model(cfg)
    network.to(rank)
    if cfg.MODEL_PARAM.MODEL_PRETRAIN_PATH != 'none':
        state = torch.load(cfg.MODEL_PARAM.MODEL_PRETRAIN_PATH, map_location='cpu')
        if 'network' in state:
            state = state['network']
        try:
            print(network.load_state_dict(state), flush=True)
        except:
            state2 = {}
            for k, v in state.items():
                state2[k[7:]] = v
            print(network.load_state_dict(state2, strict=False), flush=True)
    ...
    if cfg.TRAIN.OPTIM == 'lion':
        ...
        #     )
        # else:
            optimizer = torch.optim.AdamW(optim_params, lr=cfg.TRAIN.LR)
    ...
    lr_scheduler.step()
    if cfg.MODEL_PARAM.MODEL_PRETRAIN_PATH != 'none':
        lr_scheduler.step(cfg.TRAIN.current_epoch)
        lr_scheduler.step()
    ...
    if 'newgraph' in cfg.MODEL_NAME:
        trainDataset = handDataset2(mano_path=mano_path,
                                   ...
                                   aux_size=IMG_SIZE // aux_lambda, data_type=cfg.DATASET.DATA_TYPE)
    else:
        trainDataset = handDataset(mano_path=mano_path,
                                   ...
                                   uv=[-cfg.DATA_AUGMENT.UV, cfg.DATA_AUGMENT.UV],
                                   aux_size=IMG_SIZE // aux_lambda, data_type=cfg.DATASET.DATA_TYPE)
    evalDataset = handDataset1(InterHand_dataset(cfg.DATASET.INTERHAND_PATH, split='test'))
    ...
    provider_eval = DataProvider(dataset=evalDataset, batch_size=256,
                                 num_workers=4, dist=dist_training)
    ...
    for hand_type in ['right']:
        ...
        if 'newgraph' in cfg.MODEL_NAME:
            Loss[hand_type] = ManoLoss(J_regressor, manoData['f'],
                                        level=4,
                                        device=rank)
        else:
    ...
    J_regressor_layer['left'] = J_regressor_layer['right']
    ...
    for epoch in range(cfg.TRAIN.current_epoch, cfg.TRAIN.EPOCHS):
        ...
        for bIdx in train_bar:
            ...
            if 'newgraph' in cfg.MODEL_NAME:
                ...
                root_rel, left_pose, left_shape, right_pose, right_shape] = label_list_out
            else:
                [ori_img,
                 imgTensors,
                 v2d_l, j2d_l, v2d_r, j2d_r,
            ...
            if not only_j3d:
                if 'newgraph' in cfg.MODEL_NAME:
                    loss, aux_lost_dict, mano_loss_dict, coarsen_loss_dict = \
                        mano_loss_GCN(cfg, epoch,
                                      ...
                                      v2d_l, j2d_l, v2d_r, j2d_r,
                                      v3d_l, j3d_l, v3d_r, j3d_r,
                                      root_rel, imgTensors.shape[-1],left_pose, left_shape, right_pose, right_shape,
                ...
                j3d_left = J_regressor_layer['left'](result['verts3d']['left'])
                j3d_right = J_regressor_layer['right'](result['verts3d']['right'])
                j2d_left = projection_batch(paramsDict['scale']['left'], paramsDict['trans2d']['left'], j3d_left, img_size=IMG_SIZE)
                j2d_right = projection_batch(paramsDict['scale']['right'], paramsDict['trans2d']['right'], j3d_right, img_size=IMG_SIZE)
                j3d_left = j3d_left - j3d_left[:, 0:1]
                j3d_right = j3d_right - j3d_right[:, 0:1]
                loss3d = F.smooth_l1_loss(j3d_left, j3d_l) + F.smooth_l1_loss(j3d_right, j3d_r)
                loss2d = F.mse_loss(j2d_left/ 256 * 2 - 1, j2d_l/ 256 * 2 - 1) + F.mse_loss(j2d_right/ 256 * 2 - 1, j2d_r/ 256 * 2 - 1)
                loss = loss3d + loss2d
                mano_loss_dict = {}
                aux_lost_dict = {}
                #
                # loss = F.smooth_l1_loss(j3d_l, )
            #mask, dense, hms,
            ...
            if rank == 0:
                if total_idx % 50 == 0:
                    outfile.write('epoch {} iter {} loss {} lr {} \n'.format(epoch, total_idx, loss.item(),  lr_scheduler.get_lr()[0]))
                    outfile.flush()
                for k, v in mano_loss_dict.items():
                    if k != 'total_loss':
                        if total_idx % 50 == 0:
                            outfile.write(' , train/mano_{}: {} \n'.format(k, v.item()))
                    ...
                    if k != 'total_loss':
                        if total_idx % 50 == 0:
                            for t in range(len(v)):
                                outfile.write('train/coarsen_{}_{}: {} \n'.format(k, t, v[t].item()))
                # --------
                # | tqdm |
                # --------
        ...
        if (epoch + 1) % cfg.SAVE.SAVE_GAP == 0 and epoch > 4:
            if rank == 0:  # save checkpoint in main process
                print('save model ', cfg.SAVE.SAVE_DIR, flush=True)
                try:
        ...
        if (epoch + 1) % 50 == 0 or epoch+1 == cfg.TRAIN.EPOCHS: #or epoch == 0:
            ...
            if rank == 0:
                ...
                print('left hand pampjpe : ', 1000 * np.mean(pa_joint_error), flush=True)
