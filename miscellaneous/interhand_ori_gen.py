...
class InterHand_dataset():
    def __init__(self, data_path, start, sizes , split):
        assert split in ['train', 'test', 'val']
        self.split = split
        mano_path = get_mano_path()
        self.mano_layer = {'right': ManoLayer(mano_path['right'], center_idx=None),
        ...
        self.size = sizes#len(glob(osp.join(data_path, split, 'anno', '*.pkl')))
    ...
    def __len__(self):
        ...
        #     return self.size + 1020000
        # return self.size

    def __getitem__(self, idx):
        idx = idx + self.start
        if self.extra_syns and self.split == 'train' and idx >= 366358:#self.size:
            img = cv.imread(osp.join(self.syns_path, self.split, 'color_img', '{}.jpg'.format(idx)))
            mask = cv.imread(osp.join(self.syns_path, self.split, 'color_mask', '{}.jpg'.format(idx)))
            dense = cv.imread(osp.join(self.syns_path, self.split, 'color_dense', '{}.jpg'.format(idx)))

            with open(os.path.join(self.syns_path, self.split, 'color_annot', '{}.pkl'.format(idx)), 'rb') as file:
                data = pickle.load(file)
            ...
            for hand_type in ['left', 'right']:
                ...
                for hIdx in range(7):
                    hm = cv.imread(
                        os.path.join(self.syns_path, self.split, 'color_hms', '{}_{}_{}.jpg'.format(idx, hIdx, hand_type)))
        ...
        outdict['mask'] = mask
        outdict['dense'] = dense
        outdict['hand_dict'] = hand_dict
        np.save(self.syns_path + self.split + '/all/' + '{}'.format(idx), outdict)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start", type=int, default=1200000)
    parser.add_argument("--sizes", type=int, default=30000)
    ...
    dataset = InterHand_dataset(opt.data_path, opt.start, opt.sizes, split='train')
    print('dataset length : ', len(dataset), flush=True)
    batch_generator = DataLoader(dataset, batch_size=256, shuffle=False,
                                 num_workers=8, pin_memory=False)
    idx = 0
    for itr,  targets in tqdm(enumerate(batch_generator)):
        idx += 1