...
def render_data(save_path, split, start_idx):
    from utils.vis_utils import mano_two_hands_renderer
    mano_path = get_mano_path()
    os.makedirs(osp.join(save_path, split, 'color_mask'), exist_ok=True)
    os.makedirs(osp.join(save_path, split, 'color_dense'), exist_ok=True)
    os.makedirs(osp.join(save_path, split, 'color_hms'), exist_ok=True)

    size = len(glob(osp.join(save_path, split, 'color_annot', '*.pkl')))
    ...
    print(' start idx ', start_idx, flush=True)
    for idx in tqdm(range(start_idx, start_idx + 100000)):### 366358 size 261500
        with open(osp.join(save_path, split, 'color_annot', '{}.pkl'.format(idx)), 'rb') as file:
        ...
        for hand_type in ['left', 'right']:
            ...
            for hIdx in range(len(hms)):
                cv.imwrite(os.path.join(save_path, split, 'color_hms', '{}_{}_{}.jpg'.format(idx, hIdx, hand_type)),
        ...
        cv.imwrite(osp.join(save_path, split, 'color_mask', '{}.jpg'.format(idx)), img_mask)
        ...
        cv.imwrite(osp.join(save_path, split, 'color_dense', '{}.jpg'.format(idx)), img_dense)


class InterHand_dataset():
    def __init__(self, data_path, split, start, vis=0):
        assert split in ['train', 'test', 'val']
        self.split = split
        mano_path = get_mano_path()
        self.mano_layer = {'right': ManoLayer(mano_path['right'], center_idx=None),
    ...
    def __len__(self):
        return self.size#20000#self.size
    ...
    def __getitem__(self, idx):
        ...
        if self.vis:
            left2d = hand_dict['left']['joints2d']
            right2d = hand_dict['right']['joints2d']
            fig = plt.figure()
...
if __name__ == '__main__':
    ...
    if opt.gen_anno:
        for split in ['train']:#['train', 'test', 'val']:
            render_data(opt.save_path, split, opt.start)
    else:
        idx = 0
        for split in ['train']:
            dataset = InterHand_dataset(data_path=opt.data_path, split=split, start=opt.start, vis=opt.vis)
